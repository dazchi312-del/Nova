...
path = Path("nova/core/loop.py")
src = path.read_text()
original = src

# --- Edit 1: Add import (after existing nova.core imports) ---
# Find a stable anchor: the artifact import we know exists post-Commit A
import_anchor = "from nova.core.artifact import"
if "from nova.core.embedder import NomicEmbedder" not in src:
    # Insert new import line after the artifact import line
...
# --- Verify Optional is imported (sanity check) ---
if "Optional" not in src.split("\n", 50)[0:50].__str__():
    # crude check; just warn
    if "from typing import" not in src[:2000]:
        print("WARN: 'Optional' may not be imported — check typing imports")
